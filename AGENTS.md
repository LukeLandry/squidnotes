...
## Verify
- Run `./gradlew build` for the same check CI runs. It currently covers compilation, remap, and `test`, but there are no `src/test` sources yet.
...
## Stack
- Single Gradle project using Fabric Loom `1.13.6` for Minecraft `1.21.11`.
- Java is compiled with `options.release = 21`; `fabric.mod.json` also requires Java 21. CI uses JDK 25 to run Gradle, so avoid assuming the runtime JDK version matches the bytecode target.
...
- Shift-right-click should pass through on both sides via `player.shouldCancelInteraction()` so vanilla block placement against note blocks still works.

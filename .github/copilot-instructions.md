...
This is a Fabric mod for Minecraft `1.21.11` using Java 21 and Loom's split environment source sets.
...
- The note-block flow is intentionally split across client and server:
  ...
  - Server: `NoteBlockFeature.applySelection(...)` validates the payload, updates the block state, triggers the synced block event, and records the vanilla tuning stat.
...
- For new UI strings, add translation keys to `en_us.json` and continue using `Text.translatable(...)`.
